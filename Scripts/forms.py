...
class Forms:
    ...
    def showRenameGame(self, index):
        ...
        self.newline()
        c = input(" Type in the new name for %s: " % (self.games.getname(index=index)))
        if c.lower() == "quit":
            sys.exit(0)
        elif c.lower() != "back":
    ...
    def showDeleteGame(self, index):
        ...
        self.printc("\tPath:\t\"%s\"" % (self.games.getpath(index=index)), Const.BB)
        self.newline()
    ...
    def showGamelist(self, help=False):
        ...
        for i in range(0, len(games)):
            if self.games.gethost(index=i):
                self.printc("\t[%i]\t%s [HOST]" % (i + 1, self.games.getname(index=i)), Const.G)
            else:
                self.printc("\t[%i]\t%s" % (i + 1, self.games.getname(index=i)), Const.G)

        self.newline()
        self.printc(" Additional commands:")
        self.printc("\t[%i]\tAdd new game - Enter also \"add\"" % (len(games) + 1), Const.O)
        self.printc("\t[%i]\tUninstall - Enter also \"uninstall\"" % (len(games) + 2), Const.O)
        self.printc("\t[%i]\tSettings - Enter also \"settings\"" % (len(games) + 3), Const.O)
        self.printc("\t[%i]\tQuit - Enter also \"quit\"" % (len(games) + 4), Const.O)
        self.printc("\t[%i]\tHelp - Enter also \"help\"" % (len(games) + 5), Const.O)
        self.newline()

        if help:
            self.printc(" Help page:", Const.BB)
            self.printc("\t<ID>\t\t- Entering the ID of a game, will select it", Const.BB)
            self.printc("\tadd\t\t\t- Will provide a form, to easily add an game", Const.BB)
            self.printc("\tuninstall\t- Will uninstall the Helper and all its data", Const.BB)
            self.printc("\tquit\t\t- Will instantly close the application", Const.BB)
    ...
    def showGame(self, index):
        ...
        self.printc("\t(q)uit\t\t- Will instantly close the application", Const.O)
        self.newline()
        ...
        while True:
            ...
            if action[0] == "s":
                ...
                self.printc(" Wrong action! Please enter one of the given options!", Const.R)
    ...
    def showStartGame(self, index):
        ...
        if os.path.isfile(self.games.getpath(index=index)):
            ...
            subprocess.Popen([path.__str__()], stdin=None, stderr=None, close_fds=None)
            sys.exit(0)
    ...
    def showUninstall(self):
        ...
        if hostgame is not None:
            self.printc(" The hostgame %s will be moved back to its original folder." % hostgame["name"])
            self.printc(" Current path:\t%s" % hostgame["path"], Const.O)
            self.printc(" New Path:\t%s" % Const.reldir, Const.G)
            self.newline()
        else:
            self.printc(" No hostgame found. Cant restore the original game!", Const.R)
            self.newline()

        self.printc(" The following folder an all its components will be deleted!:")
        ...
        self.printc(" Additional commands:")
        self.printc("\tback - Will go back to the Gamelist and cancel the process", Const.O)
        self.printc("\tquit - Will close the tool immediately and cancel the process", Const.O)
        self.newline()
        ...
        while True:
            ...
            if c[0] == "b":
                ...
                if os.path.isfile("%s/RemotePlayTogetherHelperUninstaller.bat" % path.__str__()):
                    ...
                    input(Const.R + " Error! Cant uninstall the Tool. Press Enter." + Const.W)
    ...
    def showUpdate(self, force=False):
        ...
        if self.settings.get(Const.settingsEnableUpdatesKey, True) or force:
            if (datetime.datetime.now() - datetime.datetime.fromtimestamp(float(self.settings.get(Const.settingsLastUpdatePollStampKey, 0)))).days >= int(self.settings.get(Const.settingsUpdateIntervalKey, 7)) or force:
                self.settings.set(Const.settingsLastUpdatePollStampKey, datetime.datetime.now().timestamp())
                if self.updater.poll():
                    self.printc(" There is a new version of the tool available!", Const.G)
                    self.newline()
                    c = input(" Do you want to download the newer version (yes|no)?: ").lower()
                    if c[0] == "y":
                        self.newline()
                        self.printc(" Updating the tool now!", Const.O)
                        self.updater.update()
                else:
                    self.printc(" There is no newer version available!", Const.R)
                    self.newline()
    ...
    def showSetup(self):
        ...
        if c[0] == "y":
            self.newline()
            self.printc(" To use this tool, you have to install a RemotePlayTogether compatibel game via steam.")
            self.newline()
            self.printc(" If you have done this, please enter the complete path to the .exe of the game.")
            ...
            while True:
                ...
                if os.path.isfile(path):
                    ...
                    self.printc(" You have entered a wrong path. Look at the example.", Const.R)
            parentpath = Path(path).parent
            modifiedpath = str(parentpath) + "RPTH_%s" % (time.strftime("%Y%m%d_%H%M%S"))
            ...
            self.printc("\t2. Copy all the gamefiles to another place and remember the .exe name.")
            self.printc(
                "\t3. Copy the RemotePlayTogether.exe to the emptied folder of the game and rename it like the remembered .exe name.")
            self.printc("\t4. Open the renamed .exe, type in \"no\" and press Enter afterwards.")
            self.printc("\t5. Now you can add games to the list.")
            self.printc("\t6. To use it, go into Steam to the Game you choose at beginning and press Play.")
    ...
    def showSettings(self):
        while True:
            ...
            self.newline()
            self.printc("\t[1] Enable updates (1=Yes|0=No):\t%i" % self.settings.get(Const.settingsEnableUpdatesKey, 1), Const.G)
            self.printc("\t[2] Update interval (in days):\t\t%i" % self.settings.get(Const.settingsUpdateIntervalKey, 2), Const.G)
            self.printc("\t[3] Last update poll (read only):\t%s" % datetime.datetime.fromtimestamp(float(self.settings.get(Const.settingsLastUpdatePollStampKey, 0))).strftime("%Y-%m-%d %H:%M:%S"), Const.G)
            ...
            if c[0] == "q":
                ...
                while True:
                    c = input(" Enter 1 for enabling or 0 for disabling updates: ")
                    if c.isdigit() and 0 <= int(c) <= 1:
                        self.settings.set(Const.settingsEnableUpdatesKey, int(c))
                        break
            elif c.isdigit() and int(c) == 2:
                while True:
                    c = input(" Enter a value for the update interval (in days): ")
                    if c.isdigit() and 0 <= int(c):
                        self.settings.set(Const.settingsUpdateIntervalKey, int(c))
                        break
            elif c.isdigit() and int(c) == 4:
                self.showUpdate(force=True)
                input(" Press Enter to continue...")
            else:
                self.printc(" Please enter a valid number or type quit or back!", Const.R)